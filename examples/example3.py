...
xmin = np.min((np.min(ex1.X,),np.min(X_BA_p5[:,idx_p5]),np.min(X_BA_p6[:,idx_p6])))
xmax = np.max((np.max(ex1.X,),np.max(X_BA_p5[:,idx_p5]),np.max(X_BA_p6[:,idx_p6])))
...
im1 = axs[1].imshow(X_BA_p5[:,idx_p5].reshape(num_pixels,num_pixels),vmin=xmin,vmax=xmax)
axs[1].set_title("BA-GMRES(5)",fontname='cmr10',fontsize=16)
plt.colorbar(im1, ax=axs[1])
im2 = axs[2].imshow(X_BA_p6[:,idx_p6].reshape(num_pixels,num_pixels),vmin=xmin,vmax=xmax)
