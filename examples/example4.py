...
xmin = np.min((np.min(ex1.X,),np.min(X_BA[:,idx]),np.min(X_BA_dp[:,idx_dp]),np.min(X_BA_ncp[:,idx_ncp])))
xmax = np.max((np.max(ex1.X,),np.max(X_BA[:,idx]),np.max(X_BA_dp[:,idx_dp]),np.max(X_BA_ncp[:,idx_ncp])))
num_pixels = ex1.num_pixels
fig, axs = plt.subplots(2,2, figsize=(10,10))
im00 = axs[0,0].imshow(ex1.X,vmin=xmin,vmax=xmax)
axs[0,0].set_title("Exact Image",fontname='cmr10',fontsize=16)
axs[0,0].axis('off')
plt.colorbar(im00, ax=axs[0,0])
im01 = axs[0,1].imshow(X_BA[:,idx].reshape(num_pixels,num_pixels),vmin=xmin,vmax=xmax)
axs[0,1].set_title("BA-GMRES($\infty$), No SR",fontname='cmr10',fontsize=16)
axs[0,1].axis('off')
plt.colorbar(im01, ax=axs[0,1])
im10 = axs[1,0].imshow(X_BA_dp[:,idx_dp].reshape(num_pixels,num_pixels),vmin=xmin,vmax=xmax)
axs[1,0].set_title("BA-GMRES($\infty$), SR = DP",fontname='cmr10',fontsize=16)
axs[1,0].axis('off')
plt.colorbar(im10, ax=axs[1,0])
im11 = axs[1,1].imshow(X_BA_ncp[:,idx_ncp].reshape(num_pixels,num_pixels),vmin=xmin,vmax=xmax)
